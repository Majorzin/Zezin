...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Causs\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=554591233629:+55 4591233629\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan o qr code'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} Demoro pra sair `
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬⌛❭ Pera ai to fazendo',
					success: '️❬ ✔ ❭ Feito com sucesso ',
					...
					error: {
				stick: '*Deu certo não*',
				Iv: 'Alguma coisa de errado no link☹️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❌',
					premium: '[❗] ESTE COMANDO É SO PARA *USUÁRIOS PREMIUMS*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD ZEZIN BOT*',
					benned: 'Você está banido, por favor, contate o proprietário para retirar o ban',
					ownerG: '[❗] Este é um recurso especial para o DONO ❌',
					ownerB: '[❗] Este é um recurso especial para o DONO ❌',
					userB: `──「 LISTA 」──\nOlá !\nDesculpe. Você não está registrado, Registre-se usando o \n\nCommando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Zezin|17\n\n──「 ZEZIN 」──`,
			...
			const ownerNumber = ["554591233629@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"554591233629@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["554591233629@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["554591233629@s.whatsapp.net"]//ubah nomor lo
			const premium = ["554591233629@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text);
		...
		if (isGroupAdmins) return reply('cara, se você não fosse adm ja tava banido')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} olha o ban chegando em 5 segundos`)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KaKaKaKaK tchau seu merda 🤙")
		}, 0)
	}
		...
		if (isGroupAdmins) return reply('vc é adm não vo da ban')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} olha o ban chegando em 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK manda link ae otario")
		}, 0)
	}
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("tmnc")){
			client.updatePresence(from, Presence.composing)
			reply("vai vc, ja me falou que gosto quando te pegaram 😳")
	}

		if (messagesC.includes("vsfd")){
			client.updatePresence(from, Presence.composing)
			reply("vamo junto entt")
	}

		if (messagesC.includes("cadebot")){
			client.updatePresence(from, Presence.composing)
			reply("ta vendo n krlh, to comendo sua mae no uno")
	}
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				...
				case 'menu1':
					...
					if (!isOwner) return reply('Só com o meu dono')
				...
				case 'walpaperhd':
                    ...
                    if (!isGroup) return reply(from, 'este comando só pode ser usado em grupo', id)
		...
		case 'bpfont':
			...
			break
                ...
                case 'travar':
                    ...
                    client.sendMessage('554591233629@s.whatsapp.net', options, text, {quoted: mek})
		   ...
		   await limitAdd(sender)
			...
			case 'instaimg':
				...
				break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('afk?')
...
break
				...
				break
					...
					if (Number(args[0]) === 1) {
						...
						client.sendMessage(from,`Atenção a todos os membros ativos deste grupo, anti-link. se você enviar um link de grupo, você vai tomar ban`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('anti-link foi desabilitado ')
